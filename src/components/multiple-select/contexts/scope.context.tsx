...
export const ScopeContextProvider = React.memo((props: ScopeContextProviderProps) => {
  ...
  const showMenu = storeSelectors.use.showMenu();
  const selectedValues = storeSelectors.use.selectedValues();
  ...
  React.useEffect(() => {
    storeSelectors.setState({ tempSelectedValues: selectedValues });
  }, [showMenu, selectedValues]);
